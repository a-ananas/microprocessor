...
\tikzstyle{registers} = [rectangle, minimum width=3cm, minimum height=1cm, text centered, draw=black]
...
\begin{document}
	...
	\begin{frame}
		...
		\begin{tikzpicture}[node distance=2cm]
			\node (start) [memory] {ROM};
			\node (wsh) [registers, below of=start] {Process};
			\node (wsh3) [empty, below of=wsh] {\dots};
			\node (wsh2) [registers, below of=wsh3] {Process Next};
			\node (ram) [memory, right of=wsh, xshift=2cm] {RAM};

			\draw [arrow] (start) -- (wsh);
			\draw [arrow] (wsh) -- (wsh3);
			\draw [arrow] (ram) -- (wsh);
			\draw [arrow] (ram) |- (wsh2);
			\draw [arrow] (ram) |- (wsh3);
			\draw [arrow] (wsh3) -- (wsh2);
		\end{tikzpicture}
	\end{frame}
