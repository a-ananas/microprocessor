...
def decodeur_dinstructions(instr: Variable):
    ...
    isU = get_isU(opcode)
    #set defaults
    immD = Constant(32 * '0') #immediat sur 32 bits
    WenableD = Constant('0')
    ...
    immR,WenableR = get_field_R(instr)
    immI,WenableI = get_field_I(instr)
    immU,WenableU = get_field_U(instr)

    imm = Mux(isU,Mux(isI,Mux(isR,immD,immR),immI),immU)

    Wenable = Mux(isU,Mux(isI,Mux(isR,WenableD,WenableR),WenableI),WenableU)

    return opcode,imm,rs1,rs2,rd,Wenable,isR,isI,isU,opcode[0],opcode[1],opcode[2],opcode[3],opcode[4]
...
def get_isR(o: Variable):
    ...
    #les codes 10 + tout sauAf 000 et 110 et 010 (celui la ne correspond a rien, donc c'est pas tres grave, mais je prefere que isR soit faux dans ce cas) 
    cUnDesTrois = ~(o[2] | o[1] | o[0]) | (o[2] & o[1] & ~o[0]) | (~o[2] & o[1] & ~o[0])
    c = o[4] & ~o[3] & ~cUnDesTrois
...
def get_field_R(instr: Variable) -> Variable:
    assert(instr.bus_size == 32)
    imm = Constant(32 * '0')
    Wenable = Constant("1") #all the R instruction are yes on wenable
    return (imm, Wenable)

def testDeco2() -> None:
    a = Input(32)
    b = isR(a)
    b.set_as_output("output")

def testDeco() -> None:
    a = Input(32)
    opcode,imm,rs1,rs2,rd,Wenable,isR,isI,isU,q,w,e,r,t= decodeur_dinstructions(a) 
