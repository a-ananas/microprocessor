...
a multiplexor (as in the ALU) using `r` as the selector to get only the value we're looking for.
...
and then another one using `wenable` as a selector so we only modify the value for the register which's address matches `rd` and only if `wenable` is set to true.
